<template>
  <div class="layui-container flex">
    <div class="latui-row font pd30">确定更新账号为：{{ username }} 吗？</div>
    <div class="layui-row">
      <button type="button" class="layui-btn" @click="submit()" :class="{'layui-btn-disabled': isSend}">确定更新</button>
      <router-link class="layui-btn layui-btn-primary" :to="{name: 'index'}">返回首页</router-link>
    </div>
  </div>
</template>
